...
| Tab | What it does |
| --- | --- |
| **Briefing** (landing page) | A one-screen summary for the Game Master, written live from the current state. It opens with a verdict on the state of the Isles (Calm Seas → In Crisis) and a short bottom line: the nearest crisis, the biggest shift since canon, and a silent 4-season forecast of what happens if nothing is done. Below that are headline tiles and one line per tab, then four short-bullet sections: threats & crises, power standings, the economy at a glance, and open threads & coming events. Each bullet says what is happening and why, using the same in-world reasons as the dispatches. |
